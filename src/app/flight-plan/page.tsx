  ...
  if (authenticated === false) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        <Zap className="w-12 h-12 text-[#006DD8] mx-auto mb-4" />
        <h1 className="text-3xl font-bold text-text-primary mb-3">Flight Plan</h1>
        ...
        <Link href="/auth"
          className="inline-flex items-center gap-2 px-6 py-3 bg-[#006DD8] text-black font-semibold rounded-lg hover:bg-[#0058B0] transition-colors">
    ...
    <div className="max-w-2xl mx-auto px-4 py-12">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-text-primary flex items-center gap-3">
            <Zap className="w-8 h-8 text-[#006DD8]" />
            Flight Plan
          </h1>
        ...
        <button onClick={generatePlan} disabled={loading}
          className="flex items-center gap-2 px-4 py-2 bg-[#006DD8] text-black text-sm font-semibold rounded-lg hover:bg-[#0058B0] transition-colors disabled:opacity-50">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <h3 className="text-lg font-bold text-text-primary font-mono">{item.ticker}</h3>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${URGENCY_COLORS[item.urgency] || URGENCY_COLORS.low}`} />
                    <span className="text-xs text-text-muted capitalize">{item.urgency}</span>
                  </div>
                </div>
                <p className="text-sm text-text-secondary mb-2">{item.rationale}</p>
                <p className="text-xs text-text-muted italic">{item.archetype_fit}</p>
              </div>
            )
          })}
        </div>
      ...
      {!plan && !loading && !error && (
        <div className="bg-surface border border-border rounded-xl p-12 text-center">
          ...
          <p className="text-text-secondary mb-4">No Flight Plan for today yet.</p>
          <button onClick={generatePlan}
            className="px-6 py-2.5 bg-[#006DD8] text-black font-semibold rounded-lg hover:bg-[#0058B0] transition-colors text-sm">
